...
class SaveButton{
    ...
    public function events(core:Core){
        ...
        saveButton.addEventListener(TriggerEvent.TRIGGER,(e)->{
            ...
            var regex = ~/(?<=\/\/ TriggerEvent\s)[\w\W]*(?=\/\/ End TriggerEvent)/gm;
            var saveContent = regex.replace(componentContent,core.eventView.codeArea.getCodeArea().text);

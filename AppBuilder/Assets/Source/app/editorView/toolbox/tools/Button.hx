...
class Button {
    ...
    public function events(core:Core){
        ...
        overlayButton.addEventListener(MouseEvent.MOUSE_UP,(e)->{
            ...
            appButton.width = overlayButton.width;
            appButton.code.width = "0.2 * core.stage.stageWidth";
            appButton.height = overlayButton.height;
            appButton.code.height = "0.1 * core.stage.stageHeight";

            appButton.x = overlayButton.x - core.editorView.appView.getAppView().x;
            trace(appButton.x);
            appButton.code.x = appButton.x+"";

            appButton.y = overlayButton.y - core.editorView.appView.getAppView().y;
            appButton.code.y = appButton.y+"";

            appButton.refresh(core);
        ...
        btn.addEventListener(TriggerEvent.TRIGGER,(e)->{
            ...
            File.saveContent(core.addProject.okButton.path + "/Source/app/"+uppercaseComponentName+".hx", appButtonAux.code.getCode());
...
import app."+uppercaseComponentName+";
");
